...
#' @export
SL.gbm <- function(Y, X, newX = X, family = gaussian(), obsWeights = NULL, gbm.trees = 10000,
                   interaction.depth = 2, shrinkage = 0.001, bag.fraction = 0.5, cv.folds = 5,
                   n.minobsinnode = 10, n.cores = NULL, ...) {
  .SL.require('gbm')
  gbm.model <- reformulate(colnames(X), "Y")
...
#' @exportS3Method predict SL.gbm
predict.SL.gbm <- function(object, newdata,...) {
  .SL.require('gbm')

  predict(object$object, newdata = newdata, n.trees = object$n.trees, type = "response")
}
