#' SuperLearner with multicore support
#'
#' `mcSuperlearner()` and `snowSuperlearner()` allow for multicore parallelization for [SuperLearner()].
#'
#' @param cluster a `cluster` object from `parallel::makeCluster()`.
#' @param mc.cores The number of cores to use, i.e. at most how many child processes will be run simultaneously. Passed to `parallel::mclapply()`.
#' @param Y,X,newX,family,SL.library,method,id,verbose,control,cvControl,obsWeights,env See [SuperLearner()].
#'
#' @details
#' `mcSuperlearner()` uses `parallel::mclapply()`, and `snowSuperlearner()` uses `parallel::parLapply()`.
#'
...
mcSuperLearner <- function(Y, X, newX = NULL, family = gaussian(), SL.library,
                           ...
                           env = parent.frame(), mc.cores = getOption("mc.cores", 2L)) {
...
#' @export
#' @rdname mcSuperLearner
snowSuperLearner <- function(cluster,
  ...
  .crossValFUN <- function(valid, Y, dataX, id, obsWeights, library, kScreen, k, p, libraryNames, verbose) {
    ...
    out <- matrix(NA, nrow = nrow(tempValid), ncol = k)
    for(s in seq(k)) {
      pred_fn = library$library$predAlgorithm[s]
      testAlg <- try(do.call(pred_fn, list(Y = tempOutcome, X = subset(tempLearn, select = tempWhichScreen[library$library$rowScreen[s], ], drop=FALSE), newX = subset(tempValid, select = tempWhichScreen[library$library$rowScreen[s], ], drop=FALSE), family = family, id = tempId, obsWeights = tempObsWeights)))
      if(inherits(testAlg, "try-error")) {
        warning(paste("Error in algorithm", library$library$predAlgorithm[s], "\n  The Algorithm will be removed from the Super Learner (i.e. given weight 0) \n" ))
        # errorsInCVLibrary[s] <<- 1
