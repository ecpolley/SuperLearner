...
#' @export
SL.svm <- function(Y, X, newX = X, family = gaussian(), type.reg = "nu-regression", type.class = "nu-classification",
  ...
  .SL.require('e1071')
	...
	if(family$family == "binomial") {
		fit.svm <- e1071::svm(y = as.factor(Y), x = X, nu = nu, type = type.class, fitted = FALSE,
		                      probability = TRUE, kernel = kernel, degree = degree, cost = cost, coef0 = coef0)
